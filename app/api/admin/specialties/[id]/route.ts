...
import { specialtyUpdateSchema } from "@/lib/validations";
...
export async function PATCH(req: Request, { params }: Ctx) {
  return handleRoute("admin.specialties.update", async () => {
    await requireAdmin();
    if (!isSameOrigin(req)) return jsonError(403, "Request origin not allowed.");
    const { id } = await params;
    if (!ID.test(id)) return jsonError(404, "Not found.");

    const parsed = specialtyUpdateSchema.safeParse(await readJson(req, 8_000));
    if (!parsed.success) {
    ...
    try {
      const item = await prisma.specialty.update({ where: { id }, data });
...
export async function DELETE(req: Request, { params }: Ctx) {
  return handleRoute("admin.specialties.delete", async () => {
    await requireAdmin();
    if (!isSameOrigin(req)) return jsonError(403, "Request origin not allowed.");
    const { id } = await params;
    if (!ID.test(id)) return jsonError(404, "Not found.");
